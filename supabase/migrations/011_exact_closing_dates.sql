-- 1. Fechas exactas de cierre y vencimiento por mes (opcional, si no está
--    se usa el día habitual de la tarjeta con la heurística <15).
ALTER TABLE credit_card_monthly_config ADD COLUMN IF NOT EXISTS closing_date DATE;
ALTER TABLE credit_card_monthly_config ADD COLUMN IF NOT EXISTS due_date     DATE;

-- 2. El trigger deja de sobreescribir el billing period cuando el frontend
--    ya lo calculó (que es siempre). Solo calcula si viene NULL.
